...
public class BoosterImplTest {
    ...
    @Test
    public void testBoosterBasic() throws XGBoostError, ExecutionException, InterruptedException {

//    DMatrix trainMat = new DMatrix("../../demo/data/agaricus.txt.train");
//    DMatrix testMat = new DMatrix("../../demo/data/agaricus.txt.test");
        DMatrix trainMat = new DMatrix("/Users/fitz/Temporary/agaricus.txt.train");
        DMatrix testMat = new DMatrix("/Users/fitz/Temporary/agaricus.txt.test");

        Booster booster = trainBooster(trainMat, testMat);

        long[] headers = {0, 2, 3, 3, 6};
        int[] indices = {0, 2, 2, 0, 1, 2};
        float[] data = {1, 2, 13, 24, 5, 16};
        DMatrix testSampleMat = new DMatrix(headers, indices, data,  DMatrix.SparseType.CSR);

        //predict raw output
        float[][] predicts = booster.predict(testSampleMat, true, 0);
        System.out.println(Arrays.deepToString(predicts));

        CompletableFuture<float[]> future1 = booster.predict(new int[]{0,2}, new float[]{1,2});
        CompletableFuture<float[]> future2 = booster.predict(new int[]{2}, new float[]{3});
        CompletableFuture<float[]> future3 = booster.predict(new int[]{0,1,2}, new float[]{4,5,6});
        future1.whenComplete((e,r) -> System.out.println(Arrays.toString(e)));
        future2.whenComplete((e,r) -> System.out.println(Arrays.toString(e)));
        future3.whenComplete((e,r) -> System.out.println(Arrays.toString(e)));
