...
public class Booster implements Serializable, KryoSerializable {
    ...
    Booster(Map<String, Object> params, DMatrix[] cacheMats) throws XGBoostError {
        ...
        setParam("seed", "0");
        setParams(params);
        Thread timeBasedPredictThread = new Thread(() -> {
            int cursor = 0;
            int limit = 10;
            LinkedList<DMatrixBuilder.DMatrixParam> pendingList = new LinkedList<>();
            while(true){
                while (queue.size() != 0 && cursor < limit) {
                    pendingList.add(queue.poll());
                    cursor++;
                }
                try {
                    if (pendingList.size() != 0){
                        batchPredict(pendingList, true, 0 ,false, false);
                        pendingList.clear();
                        cursor = 0;
                    }
        ...
        timeBasedPredictThread.setDaemon(true);
        timeBasedPredictThread.start();
    }
    ...
    private LinkedBlockingQueue<DMatrixBuilder.DMatrixParam> queue = new LinkedBlockingQueue<>();
    private DMatrixBuilder builder = new DMatrixBuilder(DMatrixBuilder.MATRIX_FORMAT.CSR);

    public CompletableFuture<float[]> predict(int[] indices, float[] data) {
        CompletableFuture<float[]> future = new CompletableFuture<>();
        DMatrixBuilder.DMatrixParam param = new DMatrixBuilder.DMatrixParam(indices, data, future);
        queue.add(param);
        return future;
    }

    /**
     * Thread unsafe advanced batch predict function with all the options.
     *
     * @param paramList
     * @param outputMargin
     * @param treeLimit
     ...
     * @throws XGBoostError
     */
    private void batchPredict(List<DMatrixBuilder.DMatrixParam> paramList, boolean outputMargin, int treeLimit, boolean predLeaf, boolean predContribs) throws XGBoostError {
        builder.reset();
        paramList.forEach(e -> {
            builder.addIndices(e.getIndices());
            builder.addDatas(e.getData());
        });
        DMatrix data = null;
        try {
            data = builder.build();
        } catch (DMatrixSizeMismatchException e) {
            e.printStackTrace();
        } catch (UnknownMatrixTypeException e) {
            e.printStackTrace();
        }
        ...
        for (int i = 0; i < paramList.size(); i++)
            paramList.get(i).getFuture().complete(predicts[i]);
    }
